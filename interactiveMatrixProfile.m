...
function [matrixProfile, profileIndex,matrixProfileSelf, profileIndexSelf] = ...
    interactiveMatrixProfile(data,data1, subLen)
...
mainWindow.profileSelfText = uicontrol('parent',mainWindow.fig, 'style', 'text',...
    'string', 'The best-so-far Self-matrix profile', 'fontsize', 10, ...
    'backgroundcolor', backColor,'horizontalalignment', 'left');

mainWindow.diffProfileText = uicontrol('parent',mainWindow.fig, 'style', 'text',...
    'string', 'Diff between Matrix and Self-Matrix', 'fontsize', 10, ...
    'backgroundcolor', backColor,'horizontalalignment', 'left');
...
set(mainWindow.profileSelfAx,'xlim',[1, dataLen]);
set(mainWindow.profileSelfAx,'ylim',[0, 2*sqrt(subLen)]);

set(mainWindow.minusProfileAx,'xlim',[1, dataLen]);
set(mainWindow.minusProfileAx,'ylim',[0, 2*sqrt(subLen)]);
...
%% preprocess for Self-matrix profile
[dataFreq, data2Sum, dataSum, dataMean, data2Sig, dataSig] = ...
    fastfindNNPre(data, dataLen, subLen); % change data to data1, data1 is the second B
idxOrder = randperm(profileLen);
matrixProfileSelf = inf(profileLen, 1);
profileIndexSelf = zeros(profileLen, 1);
...
for i = 1:profileLen
    ...
    % figure out and store the neareest neighbor
    if i == 1
        matrixProfileSelf = distanceProfile;
        profileIndexSelf(:) = idx;
    else
        updatePos = distanceProfile < matrixProfileSelf;
        profileIndexSelf(updatePos) = idx;
        matrixProfileSelf(updatePos) = distanceProfile(updatePos);
    end
    [matrixProfileSelf(idx), profileIndexSelf(idx)] = min(distanceProfile);
    ...
    if toc(timer) > 1 || i == profileLen
        ...
        hold(mainWindow.profileAx, 'on');
        prefilePlot = plot(1:profileLen, matrixProfileSelf, 'b', 'parent', mainWindow.profileSelfAx);
        hold(mainWindow.profileAx, 'off');
        ...
        mainWindow = get(mainWindow.fig, 'userdata');
        discardIdx = mainWindow.discardIdx;
        matrixProfileTemp = matrixProfileSelf;
        for j = 1:length(discardIdx)
            discardZoneStart = max(1, discardIdx(j)-exclusionZone);
            discardZoneEnd = min(profileLen, discardIdx(j)+exclusionZone);
            matrixProfileTemp(discardZoneStart:discardZoneEnd) = inf;
            matrixProfileTemp(abs(profileIndexSelf - discardIdx(j)) < exclusionZone) = inf;
        end
...
diffMatrixProfile=matrixProfile-matrixProfileSelf;
 hold(mainWindow.profileAx, 'on');
plot(1:profileLen, diffMatrixProfile, 'b', 'parent', mainWindow.minusProfileAx);
hold(mainWindow.profileAx, 'off');
...
set(mainWindow.dataAx, 'position', [30, 5*axesHeight+5*axGap+30, figPosition(3)-160, axesHeight]);
set(mainWindow.data1Ax, 'position', [30, 4*axesHeight+4*axGap+30, figPosition(3)-160, axesHeight]);
set(mainWindow.dataMx, 'position', [30, 3*axesHeight+3*axGap+30, figPosition(3)-160, axesHeight]);
set(mainWindow.profileAx, 'position', [30, 2*axesHeight+2*axGap+30, figPosition(3)-160, axesHeight]);
set(mainWindow.profileSelfAx, 'position', [30, 1*axesHeight+1*axGap+30, figPosition(3)-160, axesHeight]);
set(mainWindow.minusProfileAx, 'position', [30, 30, figPosition(3)-160, axesHeight]);

set(mainWindow.dataText, 'position', [30, 6*axesHeight+5*axGap+30, figPosition(3)-160, 18]);
set(mainWindow.data1Text, 'position', [30, 5*axesHeight+4*axGap+30, figPosition(3)-160, 18]);
set(mainWindow.mixText, 'position',[30, 4*axesHeight+3*axGap+30, figPosition(3)-160, 18]);
set(mainWindow.profileText, 'position', [30, 3*axesHeight+2*axGap+30, figPosition(3)-160, 18]);
set(mainWindow.profileSelfText, 'position', [30, 2*axesHeight+1*axGap+30, figPosition(3)-160, 18]);
set(mainWindow.diffProfileText, 'position', [30, 1*axesHeight+30, figPosition(3)-160, 18]);
